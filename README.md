...
`docs/` is a static export of the same graphs: the same UI booted in
read-only mode, generated by `kge export`. Point GitHub Pages at the `docs/`
folder of the main branch and the graphs become browsable at a plain URL —
no server, nothing to run.

- Focus, selection, and the sidebar's show/hide all still work there, and
  whatever a visitor navigates to is mirrored into the URL fragment —
...
The export stays current two ways: direnv sets `KGE_SITE_DIR=docs`, so every
Save in a locally-served editor rewrites `docs/` (commit it alongside
`graphs/`), or regenerate explicitly:
